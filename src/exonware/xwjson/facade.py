...
class XWJSON:
    ...
    def encode(self, data: Any, options: dict[str, Any] | None = None) -> bytes:
        ...
        Returns:
            Encoded bytes
        """
        # The underlying serializer takes ``options`` as a keyword-only
        # argument — calling it positionally triggers a TypeError.
        if options is None:
            return self._serializer.encode(data)
        return self._serializer.encode(data, options=options)  # type: ignore[call-arg]
    ...
    def decode(self, data: bytes, options: dict[str, Any] | None = None) -> Any:
        ...
        return self._serializer.decode(data, options=options)  # type: ignore[call-arg]
