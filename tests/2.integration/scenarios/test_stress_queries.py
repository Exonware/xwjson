...
async def test_stress_jsonpath_queries(temp_dir):
    """Stress test: High-volume JSONPath queries."""
    ...
    # Execute bounded high-volume query set (keeps runtime predictable on Windows).
    queries = [
        "$.users[*].name",
        "$.users[?(@.age > 30)].name",
        "$.users[?(@.city == 'NYC')].id",
    ...
    # Bounded workload; allow headroom for Windows + synced disks (OneDrive) and CI runners.
    max_sec = float(os.environ.get("XWJSON_STRESS_JSONPATH_MAX_SEC", "90.0"))
    assert query_time < max_sec, f"Queries too slow: {query_time:.3f}s (max {max_sec}s)"
