...
class Test5GBComparison:
    ...
    def test_encoding_comparison_stdlib_json(self, serializer, temp_dir):
        ...
        # XWJSON should be competitive (binary format should be faster)
        # Use a minimum baseline to avoid flakiness when stdlib JSON is very fast (< 1ms)
        baseline = max(json_time, 0.001)
        assert xwjson_time < baseline * 2, f"XWJSON should be competitive: {xwjson_time:.3f}s vs {json_time:.3f}s"
