...
async def test_stress_lru_cache_performance(temp_dir):
    ...
    try:
        ...
        hit_rate = stats.get("hit_rate", stats.get("hits", 0) / max(stats.get("total", 1), 1))
...
def test_stress_cache_eviction():
    ...
    try:
        ...
        assert cache.get("key100") is not None  # Still in cache
        assert cache.get("key199") is not None  # Most recent
        # Verify cache size
        stats = cache.get_stats()
        assert stats.get("size", 0) <= 100
    except ImportError:
        pytest.skip("xwnode not available")
@pytest.mark.xwjson_integration
@pytest.mark.xwjson_performance
